...
mrb_value initialize(mrb_state* M, mrb_value self) {
	...
	mrb_iv_set(M, self, mrb_intern_lit(M, "@red"), mrb_float_value(M, r));
	mrb_iv_set(M, self, mrb_intern_lit(M, "@green"), mrb_float_value(M, g));
	mrb_iv_set(M, self, mrb_intern_lit(M, "@blue"), mrb_float_value(M, b));
	mrb_iv_set(M, self, mrb_intern_lit(M, "@gray"), mrb_float_value(M, gray));
...
#define define_property(name)										\
	mrb_value get_ ## name(mrb_state* M, mrb_value const self) {	\
		return mrb_iv_get(M, self, mrb_intern_lit(M, "@" #name));	\
	}																\
																	\
	mrb_value set_ ## name(mrb_state* M, mrb_value const self) {	\
		...
		mrb_iv_set(M, self, mrb_intern_lit(M, "@" #name), ret);	\
		return ret;													\
	}																\
