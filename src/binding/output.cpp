...
mrb_value buffer(mrb_state* M, mrb_value) {
	...
	for(Output_::buffer_type::const_iterator i = buf.begin(); i != buf.end(); ++i) {
		...
		mrb_hash_set(M, h, mrb_symbol_value(mrb_intern_lit(M, "time")),
					 to_mrb(M, Output(M).local_time(i->time, true)));
		mrb_hash_set(M, h, mrb_symbol_value(mrb_intern_lit(M, "type")),
					 mrb_fixnum_value(i->type));
		mrb_hash_set(M, h, mrb_symbol_value(mrb_intern_lit(M, "message")),
					 to_mrb(M, i->message));
		mrb_hash_set(M, h, mrb_symbol_value(mrb_intern_lit(M, "screenshot")),
