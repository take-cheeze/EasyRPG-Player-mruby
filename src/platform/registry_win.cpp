...
std::string Registry::ReadStrValue(std::string const& file, std::string const& section, std::string const& key) {
	wchar_t buf[1024];
	return (GetPrivateProfileStringW(
		Utils::ToWideString(section).c_str(),
		Utils::ToWideString(key).c_str(),
		L"", buf, sizeof(buf),
		Utils::ToWideString(file).c_str()) < (sizeof(buf) - 1))
			? Utils::FromWideString(buf) : std::string();
}
