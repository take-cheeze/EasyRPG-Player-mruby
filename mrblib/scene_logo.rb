...
class Scene_Logo < Scene
  ...
  def start
    @logo = Sprite.new
    @logo_img = Bitmap.load_raw easyrpg_logo, false
    @logo.bitmap = logo_img
  end

  def transition_in
    Graphics.transition_2k Graphics::TransitionFadeIn, 24
  end

  def transition_out
    Graphics.transition_2k Graphics::TransitionFadeOut, 24
  end

  def update
    @frame_counter += 1
    Player.replace_top Scene_Title.new if
